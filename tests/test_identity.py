...
class TestTheEngineDecidesTheTransport:
    ...
    def test_the_repo_path_wins_over_everything(self, monkeypatch):
        ...
        monkeypatch.setenv(contract.ENV_AIM_URL, "aim://elsewhere:9999")
    ...
    def test_tunnel_mode_is_untouched(self, monkeypatch):
        ...
        monkeypatch.delenv(contract.ENV_AIM_URL, raising=False)
    ...
    def test_the_env_url_still_beats_the_argument(self, monkeypatch):
        monkeypatch.delenv(contract.ENV_AIM_REPO_PATH, raising=False)
        monkeypatch.setenv(contract.ENV_AIM_URL, "aim://env:1")
...
def test_the_url_env_name_comes_from_the_contract(monkeypatch):
    """Every test above monkeypatches through contract.ENV_AIM_URL, so they
    would follow a rename rather than catch a mismatch. This one pins that the
    resolver reads the name the contract declares, which is what the engine
    writes: the two agreeing is the whole point of vendoring the file.
    """
    monkeypatch.delenv(contract.ENV_AIM_REPO_PATH, raising=False)
    monkeypatch.setenv(contract.ENV_AIM_URL, "aim://declared:4242")
    assert resolve_aim_url(None) == "aim://declared:4242"
    assert contract.ENV_AIM_URL == "ALIDADE_AIM_URL"
