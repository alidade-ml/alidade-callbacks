...
- **Training repos**: bump the pin to v2.0+ in `requirements.txt`. No code changes required. The callback auto-detects the transport mode via the engine-injected `ALIDADE_AIM_REPO_PATH` env var. Existing v1.x pins continue to work in tunnel mode even when the NUC has opted into local-aim mode (mixed-version rollout is supported).
